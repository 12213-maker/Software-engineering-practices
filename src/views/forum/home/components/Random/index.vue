<template>
	<div class="testimonial-container">
		...
		<p class="testimonial">{{ infoValue.text }}</p>
		<div class="user">
			<img :src="infoValue.photo" alt="user" class="user-image" />
			<div class="user-details">
				<h4 class="username">{{ infoValue.name }}</h4>
				<p class="role">{{ infoValue.position }}</p>
			</div>
		</div>
	</div>
</template>
...
function updateTestimonial() {
	infoValue.value = testimonials[idx];
	idx++;
