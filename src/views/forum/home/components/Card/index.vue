<template>
	<div class="card" v-for="item in tab" :key="item.label">
		<div class="card-header animated-bg" id="header"><img :src="item.cities[0].url" alt="" />&nbsp;</div>
		<div class="card-content">
			<h3 :class="{ 'card-title': true, 'animated-bg': item.label ? false : true, 'animated-bg-text': true }" id="title">
				{{ item.label }}&nbsp;
			</h3>
			<p class="card-excerpt" id="excerpt">{{ item.cities[0].des }} &nbsp;</p>
			<div class="author">
				<div class="profile-img animated-bg" id="profile_img">
					<img src="https://randomuser.me/api/portraits/men/45.jpg" alt="" />&nbsp;
				</div>
				<div class="author-info">
					<strong :class="{ 'animated-bg': item.cities[0].name ? false : true, ' animated-bg-text': true }" id="name"
						>{{ item.cities[0].name }}&nbsp;</strong
					>
					<small :class="{ 'animated-bg': item.cities[0].tourists ? false : true, ' animated-bg-text': true }" id="date"
						>{{ item.cities[0].tourists }}&nbsp;</small
					>
				</div>
			</div>
		</div>
