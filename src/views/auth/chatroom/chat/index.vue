<template>
	<div>
		...
		<div style="height: 60px; background-color: #fcfcfc">
			<template v-if="!isEmpty(props.currentChatFriendId)">
				<span style="line-height: 60px; margin-left: 20px; font-size: 18px">
					{{ props.friends[props.currentChatFriendId].remark }}
				</span>
			</template>
			<template v-else-if="!isEmpty(props.currentChatGroupId)">
				<span style="line-height: 60px; margin-left: 20px; font-size: 18px">
					{{ props.groups[props.currentChatGroupId].groupName }}
				</span>
				<span style="line-height: 60px; margin-left: 20px; font-size: 12px; color: #797979"> 当前在线人数：4 </span>
			</template>
		</div>
		...
		<div class="msg-container">
			<template v-if="!isEmpty(props.currentChatFriendId)">
				<div
					v-for="(item, index) in props.imMessages[props.currentChatFriendId]"
					:class="['msg-one', item.fromId === currentUser.id ? 'message-right' : 'message-left']"
					:key="index"
				>
					<div>
						<img
			...
			<template v-else-if="!isEmpty(props.currentChatGroupId)">
				<div
					v-for="(item, index) in props.groupMessages[props.currentChatGroupId]"
					:class="['msg-one', item.fromId === currentUser.id ? 'message-right' : 'message-left']"
					:key="index"
				>
					<div>
					...
					<div class="msg-date" style="margin-left: 5px; margin-right: 5px" v-if="!isEmpty(item.createTime)">
						{{ item.createTime }}
					</div>
				</div>
			</template>
		...
		<div style="height: 180px">
			<div style="padding: 10px 15px; display: flex; height: 25px">
				<el-popover placement="top-start" display-directive="show" trigger="click">
					<template #trigger>
						<div class="myEmoji">
							<svg viewBox="0 0 1024 1024" width="25" height="25">
								<path d="M523.9 511.98m-419.5 0a419.5 419.5 0 1 0 839 0 419.5 419.5 0 1 0-839 0Z" fill="#FFD629"></path>
								<path
								...
								<path
									...
									opacity=".2"
								></path>
								<path d="M616.9 326.38m-52.7 0a52.7 52.7 0 1 0 105.4 0 52.7 52.7 0 1 0-105.4 0Z" fill="#211715"></path>
								<path
				...
				<div class="myEmoji" @click="sendPicture()">
					<svg viewBox="0 0 1024 1024" width="25" height="25">
						...
						<path
							...
							fill="#4F5D73"
						></path>
						<path d="M624.495 400.786m-41.942 0a41.942 41.942 0 1 0 83.884 0 41.942 41.942 0 1 0-83.884 0Z" fill="#F5CF87"></path>
						<path
							d="M736.34 288.941v447.383H288.958V288.941H736.34m0-55.923H288.958c-30.886 0-55.923 25.037-55.923 55.923v447.383c0 30.885 25.037 55.923 55.923 55.923H736.34c30.886 0 55.923-25.038 55.923-55.923V288.941c0-30.886-25.037-55.923-55.923-55.923z"
							fill="#FFFFFF"
						></path>
				...
				<el-popover v-model:show="data.showPopoverImage" display-directive="show" placement="top-start" trigger="click">
					<template #trigger>
			...
			<textarea @keydown="send($event)" v-model="data.msg" maxlength="1000" spellcheck="false" class="message-content">
			</textarea>
			<div class="message-send">
				<span style="color: #797979; margin-right: 15px; font-size: 12px">Ctrl+Enter：换行 | Enter：发送</span>
				<el-button @click="doSend()" type="info"> 发送 </el-button>
			</div>
		</div>
...
import { ElPopover, ElPopconfirm } from "element-plus";

const globalStore = GlobalStore();
const currentUser = globalStore.userInformation;
const props = withDefaults(
	defineProps<{
		currentChatFriendId: any;
		currentChatGroupId: any;
		friends: any;
		groups: any;
		imageList: any;
		imMessages: any;
...
const emit = defineEmits(["sendMsg", "openFriendCircle"]);
...
function sendYiyan() {
	let xhr = new XMLHttpRequest();
	xhr.open("get", "https://api.oick.cn/yiyan/api.php");
	xhr.onreadystatechange = function () {
		if (xhr.readyState === 4) {
...
function sendShehui() {
	let xhr = new XMLHttpRequest();
	xhr.open("get", "https://api.oick.cn/yulu/api.php");
	xhr.onreadystatechange = function () {
		if (xhr.readyState === 4) {
